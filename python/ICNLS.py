        ...
        if func == "prod":
            ...
            if pps == "vrs":
                ...
                def concav_rule(model, i, h):
                    ...
                    if crow[h] == 0:
                        return Constraint.Skip
                    return crow[i] * (model.a[i] + sum(model.b[i, j] * brow[j] for j in model.j)) <= crow[h] * \
                           (model.a[h] + sum(model.b[h, j] * brow[j] for j in model.j))
        ...
        if func == "prod":
            ...
            if pps == "crs":
                ...
                def qconcav_rule(model, i, h):
                    ...
                    if crow[h] == 0:
                        return Constraint.Skip
                    return crow[i] * (sum(model.b[i, j] * brow[j] for j in model.j)) <= crow[h] * \
                           (sum(model.b[h, j] * brow[j] for j in model.j))
