        ...
        if (func == "prod"):
            ...
            if (pps == "vrs"):
                ...
                def reg_rule(model, i):
                    arow   = x[i] 
                    return  model.a[i] + sum(model.b[i, j]*arow[j] for j in model.j) + model.e[i] == y[i]
                model.reg = Constraint(model.i, rule=reg_rule, doc='regression')
                    
                def concav_rule(model, i, h):
                    brow = x[i]
                    return  model.a[i] + sum(model.b[i,j]*brow[j] for j in model.j) <= model.a[h] + sum(model.b[h,j]*brow[j]  for j in model.j)
            ...
            if (pps == "crs"):
                ...
                def reg_rule(model, i):
                    arow   = x[i] 
                    return  sum(model.b[i, j]*arow[j] for j in model.j) + model.e[i] == y[i]
                model.reg = Constraint(model.i, rule=reg_rule, doc='regression')
                    
                def concav_rule(model, i, h):
                    brow = x[i]
                    return  sum(model.b[i,j]*brow[j] for j in model.j) <= sum(model.b[h,j]*brow[j]  for j in model.j)
                model.concav = Constraint(model.i, model.h, rule=concav_rule, doc='concavity constraint')         
